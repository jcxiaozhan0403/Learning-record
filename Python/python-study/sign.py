...
async def jd_sign(request: Request, background_tasks: BackgroundTasks,
    ...
    task_id = str(uuid4())
    scheduler.add_job(id=pt_pin, name=f'{pt_pin}', func=signBeanAct, kwargs={"pt_pin": pt_pin, "pt_key": pt_key},
                      trigger='cron', hour=data_time.hour, minute=data_time.minute, second=data_time.second,
...
async def token_sign(request: Request, path: str, background_tasks: BackgroundTasks,
    ...
    path_dict = {"csairSign": csairSign, "sichuanairSign": sichuanairSign, "ctripSign": ctripSign,
                 "dragon_boat_2023": dragon_boat_2023, "meituan": meituan, "weimob": weimob, "10086": m10086,
                 "10010": m10010, "dp": youzan_dp, "95516": m95516, "kraf": kraf, "erke": demogic_erke,
                 "decathlon": decathlon, "ys": ys, "juejin": juejin, "tuhu": tuhu}
    if path in path_dict.keys():
        background_tasks.add_task(path_dict[path], **{"token": token})
        scheduler.add_job(id=token, name=f'{token}', func=path_dict[path], kwargs={"token": token}, trigger='cron',
                          hour=data_time.hour, minute=data_time.minute, second=data_time.second, replace_existing=True)
...
async def cookie_sign(request: Request, path: str, background_tasks: BackgroundTasks,
    ...
    if path in path_dict.keys():
        ...
        data_time = parse(time)
        scheduler.add_job(id="", name="", func=path_dict[path], kwargs={"cookie": cookie}, trigger='cron',
                          hour=data_time.hour, minute=data_time.minute, second=data_time.second, replace_existing=True)
...
async def crontab_task(**kwargs):
    ...
    pre_list = ['jd', 'csai', 'sichuanair', 'dragon_boat_2023', 'meituan', 'weimob', '10086', '10010', 'dp', '95516',
                'kraf', 'erke', 'honda', 'decathlon', 'ys', 'juejin']
