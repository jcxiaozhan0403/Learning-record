...
磁盘分区
磁盘容量=主分区容量(最多4个)+扩展分区容量(最多1个)[总数不超过4个]1
...
- 双重输出重定向
```
命令>文件名2>文件名 (将正确错误结果放在不同或相同文件中)
命令&>文件名 (将正确错误信息保存到同一个文件中)
命令&>>文件名 (将正确错误信息保存到同一个文件中,不覆盖)
```
- 自定义结束符
```
命令<<自定义
...
3. 管道符
```
|  将符号前的进程输出，当做符号后进程的输入
```

# 第二十四次课
文件的搜索命令
1. which  搜索可执行文件(命令文件)的存放路径，有别名显示别名
```
which <被搜索的指令>
```
2. whereis  搜索程序的二进制文件、帮助文件、源文件
```
whereis [-bmsul] [-BMS 路径 -f] 文件名
...
-u 更新档案文件
-v 显示所有过程
-z 用gzip压缩或解压文件 后缀 .tar.gz
-j 用bzip压缩或解压文件(效率高一些) 后缀 .tar.bz
-f 指定档案文件名，只能作为最后一个选项，后接档案名
-C <目录> 放到指定目录
前5个选项只能同时使用一个
```
...
**脚本文件**：将常用的linux命令存储在一个文件中，shell可以读取并执行其中的命令
...
```
```
默认使用bash解析，如果要用别的解释器解析，需在脚本文件第一行输入#!/解释器路径
```
