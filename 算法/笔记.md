...
1. 从文本串的第一个字符开始，依次与模式串的第一个字符、第二个字符、第三个字符……逐个比较。
2. 如果当前比较的字符匹配，则继续比较下一个字符；如果不匹配，则将文本串向后移动一个字符，重新从文本串的下一个字符开始比较。
3. 如果模式串的所有字符都与文本串匹配，则找到了模式串在文本串中的位置；否则，继续从文本串的下一个位置开始比较。
4. 如果文本串的最后一个字符都与模式串的最后一个字符不匹配，则确定模式串不在文本串中出现。
...
1. KMP 是一个解决模式串在文本串是否出现过，如果出现过，返回最早出现的位置的经典算法
...
3. KMP 方法算法就利用之前判断过信息，通过一个 next 数组，保存模式串中前后最长公共子序列的长度，每次回溯时，通过 next 数组找到，前面匹配过的位置，省去了大量的计算时
...
故串"ABCDABD"的next数组为`[0,0,0,0,1,2,0]`；
...
当我们遍历二叉树时，通常需要使用递归的方式来实现，这样在遍历深度很深、二叉树结构复杂时，会占用较多的栈空间而导致程序栈溢出。而线索化二叉树是在原有的二叉树上，将所有的空指针指向该节点在中序遍历中的前驱和后继节点，从而避免了递归、减少了空间开销，使得二叉树的遍历变得更加高效。

在线索化二叉树中，每个节点有两个指针：left指向左子树，right指向右子树。如果节点没有左子树，则将left指向该节点的前驱节点；如果节点没有右子树，则将right指向该节点的后继节点。通过这种方式，我们可以快速定位到某个节点在中序遍历序列中的前驱和后继节点，而不需要重复遍历二叉树。

> 图示

![中序线索二叉树](D:\Study\Learning-record\算法\中序线索二叉树.jpg)
...
大顶堆是一种二叉树结构，其中每个节点的值都大于或等于其子节点的值。具体来说，对于一个大顶堆的根节点，它的值是堆中的最大值。在一个大顶堆中，任何一个节点的值都不会小于其子节点的值。这种数据结构常用于堆排序、优先队列等应用中。

`arr[i]>=arr[2*i+1] && arr[i]>=arr[2*i+2]`

![大顶堆](D:\Study\Learning-record\算法\大顶堆.jpg)

> 小顶堆

小顶堆也是一种二叉树结构，其中每个节点的值都小于或等于其子节点的值。具体来说，对于一个小顶堆的根节点，它的值是堆中的最小值。在一个小顶堆中，任何一个节点的值都不会大于其子节点的值。小顶堆与大顶堆类似，也常用于堆排序、优先队列等应用中。

`arr[i]<=arr[2*i+1] && arr[i]<=arr[2*i+2]`

![小顶堆](D:\Study\Learning-record\算法\小顶堆.jpg)
...
- 给定 n 个权值作为 n 个叶子结点，构造一棵二叉树，若该树的带权路径长度(wpl)达到最小，称这样的二叉树为**最优二叉树**，也称为**赫夫曼树(Huffman Tree)**
- 赫夫曼树是带权路径长度最短的树，权值较大的结点离根较近

> 重要概念

1. 路径和路径长度：在一棵树中，从一个结点往下可以达到的孩子或孙子结点之间的通路，称为路径。路径中连线的数量称为路径长度。
2. 若规定根结点的层数为 1，则从根结点到第 L 层结点的路径长度为 L-1
3. 结点的权及带权路径长度：若将树中结点赋给一个有着某种含义的数值，则这个数值称为该结点的权。结 点的带权路径长度为：从根结点到该结点之间的路径长度与该结点的权的乘积==（节点的带权路径长度 = 权值 * 路径长度）==
4. 树的带权路径长度：树的带权路径长度规定为所有叶子结点的带权路径长度之和，记为 WPL(weighted path length)，权值越大的结点离根结点越近的二叉树才是最优二叉树。
5. WPL 最小的就是赫夫曼树

> 赫夫曼树的构建过程

1. 将数列中的元素按照从小到大升序排列，然后将每个元素看作一个节点。
2. 选取数列中最小的两个元素，将它们合并成一个新节点，新节点的权重等于这两个元素的权重之和。这个新节点代表的区间就是这两个元素的区间的并集。这个新节点作为一棵子树插入到原来的树中，作为新的节点。
3. 重复步骤 2，直到所有的元素都被合并成一个节点为止。
4. 最终得到的树就是赫夫曼树，它的根节点代表的区间就是整个数列的区间的并集。
5. 如果数列中存在多个相同元素，那么由于排序的问题，可以构建出多棵不同的赫夫曼树，但是它们的WPL是相同的

![赫夫曼树构建](D:\Study\Learning-record\算法\赫夫曼树构建.jpg)
...
- 赫夫曼编码是一种前缀编码，也就是说，任何一个字符的编码都不是另一个字符编码的前缀。这样可以避免解码时产生歧义，使得编码更加高效。

> 基于赫夫曼树得到赫夫曼编码

- 对于左子树，将其对应的编码设为当前编码加上0；
- 对于右子树，将其对应的编码设为当前编码加上1；
- 对于叶子节点，将其对应的编码作为该符号的编码。

![赫夫曼编码](D:\Study\Learning-record\算法\赫夫曼编码.jpg)
...
图是由节点（也称为顶点或点）和边组成的一种数据结构。节点表示实体，边表示节点之间的关系。图可以用于模型化现实世界中许多复杂的关系和问题，比如社交网络、道路和交通流、通信网络、电路板以及计算机程序中的依赖关系等等。图可以分为有向图和无向图两种类型，有向图中的边有方向，表示只能从一个节点到另一个节点，而无向图中的边没有方向，表示两个节点之间的关系是双向的。

> 常见概念

1. 顶点(vertex): 图中的一个节点或位置。
2. 边(edge): 顶点之间的连接线或关系。
3. 路径: 通过一系列的边从一个顶点到达另一个顶点的序列。
4. 无向图: 由顶点和边组成的图，边没有方向性。
5. 带向图: 由顶点和有方向的边组成的图。
6. 带权图: 顶点之间的边具有关联的权重或值。也称为网
...
![邻接矩阵](D:\Study\Learning-record\算法\邻接矩阵.jpg)
...
![邻接表](D:\Study\Learning-record\算法\邻接表.jpg)
...
5. 查找结点 v 的 w 邻接结点的下一个邻接结点，转到步骤 3。 6) 分析图
